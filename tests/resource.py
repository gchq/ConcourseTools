...
class TestResource(concoursetools.ConcourseResource[TestVersion]):
    ...
    def fetch_new_versions(
        self, previous_version: TestVersion | None = None
    ) -> list[TestVersion]:
        if previous_version:
    ...
    def download_version(
        ...
        build_metadata: concoursetools.BuildMetadata,
        file_name: str = "README.txt",
    ) -> tuple[TestVersion, Metadata]:
        print("Downloading.")
...
ConcourseMockVersion._un_flatten_functions = copy(
    ConcourseMockVersion._un_flatten_functions
)
...
class ConcourseMockResource(concoursetools.ConcourseResource[ConcourseMockVersion]):
    ...
    def fetch_new_versions(
        self, previous_version: ConcourseMockVersion | None = None
    ) -> list[ConcourseMockVersion]:
    ...
    ) -> tuple[ConcourseMockVersion, Metadata]:
        raise NotImplementedError
